...
for img in os.listdir(path):
    ...
    #print(imageName)
    jpgPath = r"C:\Users\hmzsh\Pictures\needsWork\problem6\{}.jpg".format(imageName)
    image = cv2.imread(jpgPath)
    fileExt  = os.path.splitext(img)[1]
    if fileExt == ".jpg":
        newTextpath = r"C:\Users\hmzsh\Pictures\combined\K\{}.txt".format(imageName)
        f = open(newTextpath,'r')
        lines = f.readline()
        ...
        print("Before - ", lines)
        print("normalized - A: {} B: {} C: {} D: {}".format("{:.6f}".format(A),"{:.6f}".format(B),"{:.6f}".format(C),"{:.6f}".format(D)))

        pathTosave = r"C:\Users\hmzsh\Pictures\needsWork\problem6_sol\{}.jpg".format(imageName)
        cv2.imwrite(pathTosave,image)
        newTxtpath = r"C:\Users\hmzsh\Pictures\needsWork\problem6_sol\{}.txt".format(imageName)
